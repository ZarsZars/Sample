########################################################################################################################################################
#THIS SCRIPT IS TO PULL POOL INFORMATION FROM LIST OF F5 SUPPLIED BY USER
#INPUT
    #create a list of F5 to feed into this script (inventory_Path="inventory.txt")
    #Provide username and password respectively to variable 'user' and 'password" in the script
#OUTPUT
    #Once script run successfully, it will create 2 files
     #1st file contain pool member information (poolInfoFile='PoolInformation.csv')
     #2nd file contain failed F5 (failedF5File='failedF5.csv')
########################################################################################################################################################
...
#DECLARE FUNCTION TO EXTRACT HOSTNAME FOR FEED FILE
def extractHostname():
